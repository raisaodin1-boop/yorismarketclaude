-- Bind automated loyalty credits to the selected catalog pack and to the
-- exact XAF amount confirmed by Paynote. Client-authored purchase snapshots
-- remain untrusted until all three records agree.
...
BEGIN
  ...
  v_base_points := ROUND(COALESCE(v_pack.points, 0))::integer;
  v_expected_points :=
    v_base_points
    + ROUND((v_base_points::numeric * COALESCE(v_pack.bonus_pct, 0)::numeric) / 100)::integer;
  ...
  IF ROUND(COALESCE(v_purchase.prix_fcfa, 0))::numeric(14,2) <> v_expected_amount
     OR ROUND(COALESCE(v_purchase.points, 0))::integer <> v_expected_points THEN
