...
// ─────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────
// COMPOSANT : SUIVI DE LIVRAISON EN TEMPS RÉEL
// ─────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────
// COMPOSANT : MODAL DEMANDE DE LIVRAISON
// ─────────────────────────────────────────────────────────────
function ModalDemandeLivraison({ user, userData, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1️⃣ Générer un code de suivi unique
      const random = Math.random().toString(36).substring(2, 7).toUpperCase();
      const timestamp = Date.now().toString(36).slice(-3).toUpperCase();
      const code = "YX-" + random + timestamp;

      // 2️⃣ Créer l'entrée dans la table deliveries
      ...
      // 3️⃣ Préparer le message WhatsApp pour le support Yorix
      const urgenceLabel = {
        ...
        <div className="modal" style={{ maxWidth: 480, textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            ...
            <button
              onClick={() => {
                onClose();
                // Le code est déjà dans le tracker si on recharge la page livraison
        ...
        <div style={{
          ...
          <div>
            <h3 style={{
              ...
              color: "var(--ink)", margin: 0, letterSpacing: "-.3px",
            }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              background: "rgba(252,209,22,.12)", borderRadius: "50%",
            }}/>
            <div style={{ position: "relative", zIndex: 2 }}>
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: ".72rem", opacity: .8, marginBottom: 3 }}>
                    CODE DE SUIVI
                  </div>
                  <div style={{
                    fontFamily: "'Syne',sans-serif", fontSize: "1.3rem", fontWeight: 800,
                ...
                {delivery.statut === "livre" ? (
                  <span style={{
                    ...
                    borderRadius: 50, fontSize: ".78rem", fontWeight: 700,
              ...
              <div style={{
                fontSize: ".7rem", opacity: .75, marginTop: 6,
              }}>
          ...
          {delivery.statut !== "livre" && (
            <div style={{
              ...
              >
                {/* Route courbée */}
                <path
                  d="M 30 90 Q 150 20, 300 60 T 570 40"
                  stroke="#94a3b8"
                  strokeWidth="4"
                  strokeDasharray="8 6"
                  fill="none"
                ...
                <path
                  ...
                  strokeWidth="5"
                  fill="none"
                  strokeDasharray={`${progressPct * 6} 1000`}
                  style={{ transition: "stroke-dasharray 1s ease" }}
                />
                {/* Point départ (vendeur) */}
                <circle cx="30" cy="90" r="10" fill="#ef4444" />
                <text x="30" y="115" textAnchor="middle" fontSize="10" fill="#64748b" fontWeight="600">
                  Vendeur
                </text>
                {/* Point arrivée (client) */}
                <circle cx="570" cy="40" r="10" fill="#22c55e" />
                <text x="570" y="22" textAnchor="middle" fontSize="10" fill="#64748b" fontWeight="600">
                  Vous
                </text>
                {/* Livreur animé */}
                <g style={{
              ...
              <div style={{
                display: "flex", justifyContent: "space-between",
                fontSize: ".72rem", color: "#475569", fontWeight: 600,
                marginTop: 8,
              }}>
          ...
          <div style={{ padding: "20px 22px" }}>
            ...
            <div style={{ position: "relative" }}>
              {STEPS.map((step, idx) => {
                ...
                const timestamp = delivery[`${step.id === "commande_recue" ? "commande" : step.id === "livre" ? "livre" : step.id}_at`];

                return (
                  <div key={step.id} style={{
                    display: "flex", gap: 14, marginBottom: isLast ? 0 : 14,
                    position: "relative",
                  }}>
                    {/* Ligne verticale */}
                    {!isLast && (
                    ...
                    <div style={{ flex: 1, paddingTop: 4 }}>
                      <div style={{
                        ...
                        fontSize: ".88rem", color: isDone || isCurrent ? "var(--ink)" : "var(--gray)",
                      }}>
                        {step.label}
                        {isCurrent && (
                          <span style={{
                      ...
                      <div style={{
                        fontSize: ".72rem", color: "var(--gray)", marginTop: 2,
                      }}>
                        {step.description}
                      </div>
                      {timestamp && (isDone || isCurrent) && (
                        <div style={{
                          fontSize: ".68rem", color: "var(--green)",
