...
describe("momo-status wiring", () => {
  it("uses ensureMomoOrdersPaid and does not blind-update cancelled orders", () => {
    expect(momoStatusSource).toContain('import { ensureMomoOrdersPaid } from "./_lib/momo_orders.js"');
    expect(momoStatusSource).toContain("ensureMomoOrdersPaid");
  ...
  it("repairs already-paid journals instead of early-returning without order sync", () => {
    expect(momoStatusSource).toContain('if (tx.status === "paid")');
    expect(momoStatusSource).toContain("respondPaidOrKeepPolling");
  });
});
